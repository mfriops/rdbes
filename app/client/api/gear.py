...
class GearService:
    ...
    base_url :
        ...
        Can also be supplied via env-var ``GEAR_API_URL``.
    timeout :
        Per-request timeout in seconds (default **10**).
    """

    def __init__(self, base_url: Optional[str] = None, timeout: int = 10) -> None:
        # self.base_url = (base_url or os.getenv("GEAR_API_URL") or "").rstrip("/")
        print(base_url)
        self.base_url = base_url
        if not self.base_url:
            raise ValueError("base_url not provided and GEAR_API_URL not set")
    ...
    def get_fishing_gear(self, fishing_gear_nos: Union[int, Sequence[int]]) -> Dict[str, Any]:
        # normalise to list[int] so we can join below
        if isinstance(fishing_gear_nos, str):
            ids: List[str] = [fishing_gear_nos]
        else:
            ids = list(fishing_gear_nos)

        if not ids:
            raise ValueError("fishing_gear_nos may not be empty")

        # build “…?fishing_gear_no=1,2,3”
        query = ",".join(str(i) for i in ids)
        endpoint = f"/fishing_gear?fishing_gear_no={query}"
        return self._get_json(endpoint)


    def get_isscfg(self, isscfg_nos: Union[int, Sequence[int]]) -> Dict[str, Any]:
        # normalise to list[int] so we can join below
        if isinstance(isscfg_nos, str):
            ids: List[str] = [isscfg_nos]
        else:
            ids = list(isscfg_nos)

        if not ids:
            raise ValueError("isscfg_nos may not be empty")

        # build “…?isscfg_no=1,2,3”
        query = ",".join(str(i) for i in ids)
        endpoint = f"/isscfg?isscfg_no={query}"
        return self._get_json(endpoint)
